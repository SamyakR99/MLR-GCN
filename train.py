...
def main():
    ...
    # optim = torch.optim.Adam(sgd_polices,
    #                         lr=cfg.OPTIM.LR,
    ...
    if args.resume is not None:
        if os.path.exists(args.resume):
            ...
            # filtered_state_dict = {k: v for k, v in checkpoint['state_dict'].items() if '.gc' not in k}
            # filtered_state_dict = {f"module.{k}" if not k.startswith("module.") else k: v for k, v in checkpoint['state_dict'].items() if '.gc' not in k}
            # filtered_state_dict = {k[len("module."):] if k.startswith("module.") else k: v for k, v in checkpoint['state_dict'].items() if '.gc' not in k}
            # filtered_state_dict = {f"module.{k}": v for k, v in checkpoint['state_dict'].items()}
