...
class GraphConvolution(nn.Module):
    ...
    def forward(self, input, adj):
        ## Comment for Image feature, uncomment when updating probabilites
...
class DualCoop(nn.Module):
    ...
    def forward(self, image, cls_id=None):
        ...
        b, c, _ = output.shape
        output_neg = output[:,  :c // 2, :]
        output_half = output[:,  c // 2:]
        w_half = F.softmax(output_half, dim=-1)
        w_half_neg  = F.softmax(output_neg, dim=-1)
        w = torch.cat([w_half_neg, w_half], dim=1)
        
        image_features_norm_repeated = image_features_norm.unsqueeze(1).repeat(1, w.shape[1], 1, 1)
        w_repeated = w.unsqueeze(2).repeat(1, 1, image_features_norm.shape[1], 1)

        img_feat = (image_features_norm_repeated*w_repeated).sum(-1)
        # identity = img_feat
        
        resized_output_neg = img_feat[:,:c//2] ## 32,103 
        resized_output_pos = img_feat[:,c//2:]
        ...
        img_feat = torch.cat((tf1,tf2), dim = 1)
        # breakpoint()
        text_feat = text_features.unsqueeze(0).repeat(img_feat.shape[0],1,1)
        final_out = (img_feat * text_feat).sum(-1)
        
        logits = 5*final_out.resize(b, 2, c//2)
