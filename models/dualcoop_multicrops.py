...
class DualCoop(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if cfg.DATASET.NAME == 'voc2007':
            ...
            self.gc1 = GraphConvolution(1, 25)
            self.gc2 = GraphConvolution(25, 100)
            self.gc3 = GraphConvolution(100, 200)
            self.gc4 = GraphConvolution(200, 25)
            self.gc5 = GraphConvolution(25, 1)
            # self.gc2 = GraphConvolution(10, 20)
            # self.gc3 = GraphConvolution(20, 1)
            self.relu = nn.LeakyReLU(0.2)
            self.relu2 = nn.LeakyReLU(0.2)
        ...
        if cfg.DATASET.NAME == 'unimib':
            ...
            self.relation = torch.load('/home/samyakr2/SHOP/ARK/co_occurrence_matrix_coco.pth')
            _ ,max_idx = torch.topk(self.relation, len(self.relation))
            mask = torch.ones_like(self.relation).type(torch.bool)
            for i, idx in enumerate(max_idx):
                mask[i][idx] = 0
    ...
    def forward(self, image, segments, cls_id=None):
        ...
        for segm in segments:
            # breakpoint()
            seg_features, attn_weights = self.image_encoder(segm.type(self.dtype))
            seg_features_norm = seg_features / seg_features.norm(dim=1, keepdim=True)
            segmented_features.append(seg_features_norm)
        ...
        img_output = 20 * F.conv1d(image_features_norm, text_features[:, :, None])
        seg_output = 0
        for seg_feat in segmented_features:
            output_segs = 20 * F.conv1d(seg_feat, text_features[:, :, None])
            seg_output = seg_output + output_segs
        seg_output = seg_output/len(segmented_features)
        
        ## samyak todo: make 0.8 and 0.2 args 
        output = 0.85 * img_output + 0.15 * seg_output
        ...
        tf1 = self.relu2(tf1)
        tf1 = self.gc4(tf1, self.gcn_relation.cuda())
        tf1 = self.relu2(tf1)
        tf1 = self.gc5(tf1, self.gcn_relation.cuda())
        ...
        tf2 = self.relu2(tf2)
        tf2 = self.gc4(tf2, self.gcn_relation.cuda())
        tf2 = self.relu2(tf2)
        tf2 = self.gc5(tf2, self.gcn_relation.cuda())
...
def dualcoop(cfg, classnames, **kwargs):
    ...
    print('DUDE YOU FORGOT TO UNCOMMENT THE DATAPARALLEL PART, CHECK models.dualcoop last line')
